...
!(function( factory ){
    if( typeof define === 'function' ){
        ...
        var exports = null;
        var module = { exports:null, uri:'' };
        var object = factory( require, exports, module );
        if( object ) module.exports = object;
        window['UploaderBase'] = module.exports;
    }
     ...
     */
    var xxlUploader = window.xxlUploader || require('./uploader');
    var xxlAbsoluteSwf = module.uri.substring(0, module.uri.lastIndexOf('/'))+'/uploader.swf';
    ...
    function UploaderBase(config) {
        ...
        if(!xxlUploader || !xxlUploader['hasFlash']()) this.config.isFlash = false;
    ...
    $.extend(UploaderBase.prototype,{
        ...
        remove:function( ids ){
            ...
            self.fire( UploaderBase.event.REMOVE, temps );
        },
        /**
         * 移除队列中所有的文件
         */
        ...
        _renderButton:function () {
            ...
            if(self._get('isFlash')){
                ...
                new xxlUploader.Uploader({
                    ...
                    .on(xxlUploader.EVENT.PROGRESS, function(flashId, fileName, uploadScale, scaleInfo, uploadSpeed, residualTime, currLoaded, totalSize){
                        self.fire(UploaderBase.event.PROGRESS, {fileName:fileName, uploadScale:uploadScale, scaleInfo:scaleInfo, uploadSpeed:uploadSpeed, residualTime:residualTime, currLoaded:currLoaded, totalSize:totalSize});
                    })
                    .on(xxlUploader.EVENT.COMPLETE, function(flashId, rsData, fileName, fileSize, uploadSeconds){
                        self.fire(UploaderBase.event.SUCCESS, JSON.parse(rsData));
                    })
                    .on(xxlUploader.EVENT.ERROR, function(flashId, type, desc, fileName, maxsize, filesize){
                        self.fire(UploaderBase.event.SUCCESS, {type:type, desc:desc, fileName:fileName, maxsize:maxsize, filesize:filesize});
                    });
        ...
        _select:function (ev) {
            ...
            if( files.length > 0 ){
                $.each(files, function (key,file) {
                    ...
                    file.id = UploaderBase.getGuId('f_',8);
                });
                //self.fire( UploaderBase.event.SELECT, files );
                //self._dealWithFiles();
            }else if(  value.length > 0 ){
                //b<=ie9,only one file
                console.log(value);
                files.push({
                    id: UploaderBase.getGuId('f_',8),
                    size: 0,
            ...
            if(self.queue.length>0){
                self.fire( UploaderBase.event.SELECT, self.queue );
                self._dealWithFiles();
            }

            if(errorFiles.length>0){
                self.fire( UploaderBase.event.ERROR, errorFiles );
            }
        ...
        _renderUploaderCore:function( files ){
            ...
            self.fire(UploaderBase.event.START, []);
        ...
        _html5Upload: function(  ){
            ...
            $.each(files,function( i, file ){
                ...
                self.fire( UploaderBase.event.START, [file] );
        ...
        _iframeUpload: function(  ){
            ...
            self.fire( UploaderBase.event.START , files );
        ...
        _success:function ( data, fileId ) {
            ...
            self.fire( UploaderBase.event.SUCCESS, data );
        ...
        _completes:function(){
            ...
            //self.queue = [];
            //self.target[0].value = '';
            self.fire( UploaderBase.event.COMPLETES );
        },
        _error: function(data){
            var self = this;
            self.fire(UploaderBase.event.ERROR,data)
        }
    });
     ...
     * @param config
     */
    UploaderBase.prototype.resetConfig = function ( config ){
        this.config = $.extend( this.config , config );

        /**
         * 由于Flash没有提供重新配置参数的方法 故重写NEW一下。
         * @type {UploaderBase}
         */
        if(this.config.isFlash){
            new xxlUploader.Uploader({
                flashId : self.placeholder,
                swfUrl : self.config.swfUrl,
                postUrl : self.config.postUrl,
                postData : self.config.postData,
                fieldName : self.config.fileName,
            ...
            this.target.addClass('uploaderBtn').find('object,input').css({
                'position': 'absolute',
                'right': 0,
                'top': 0,
                'width': '100%',
